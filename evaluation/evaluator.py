...
class RAGEvaluator:
	...
	def __init__(self, rag: MultiAgentSystem, neo4j_manager: Neo4jManager):
		self.rag = rag
		self.neo4j = neo4j_manager
		self.client = get_llm(0.0)

		# Context recall
		self.context_recall_chain = self._build_context_recall_chain()

		# Faithfulness
		self.decompose_chain = self._build_decompose_chain()
		self.verify_no_context_chain = self._build_no_context_verify_chain()
		self.verify_context_chain = self._build_context_verify_chain()

		# Answer Correctness
		self.breakdown_chain = self._build_breakdown_chain()
	...
	def _build_context_recall_chain(self):
		system_prompt = """Goal: Determine whether the ground-truth answer is supported by the retrieved context.

You must interpret the answer relative to the question and the narrative context.

Examples:
- Question: "Where does the hero live?"
  GT: "the forest"
  → interpret as:
  "The hero lives in the forest."

- Question: "What test must the hero overcome?"
  GT: "crossing the river"
  → interpret as:
  "The hero must overcome the test of crossing the river."

- Question: "How many helpers are there?"
  GT: "2"
  → interpret as:
  "There are 2 helpers."

IMPORTANT — short ground truths:
...
then combine it with the question to reconstruct the intended factual statement before evaluating attribution.

IMPORTANT — graph / structured retrieval:
The context may contain structured or graph-style retrieval outputs such as:
- {{'count(p)': 2}}
- {{'name': 'Arthur'}}
- {{'test': 'crossing the river'}}
- {{'location': 'the forest'}}

Interpret these results semantically in relation to the question.
...
"""
		...
		return prompt | self.client.with_structured_output(_AttributionResult)
	
	def _build_no_context_verify_chain(self):
		verify_prompt = """Goal: Judge whether each statement is acceptable when NO external evidence context exists.
...
"""
		...
		return prompt | self.client.with_structured_output(_Verification)
	
	def _build_context_verify_chain(self):
		verify_prompt = """Goal: Judge the faithfulness of each statement using the provided context and question.
...
Return exactly one verdict and one reasoning item per statement.
	...
	def evaluate_context_recall(
		...
		result = self.context_recall_chain.invoke({
			"question": question,
			"context": context_str,
			"ground_truth": ground_truth,
		})

		result = cast(_AttributionResult, result)
	...
	def _build_decompose_chain(self):
		decompose_prompt = """Goal: Given a question and an answer, break the answer into meaningful factual or semantic statements.
...
"""
		...
		return prompt | self.client.with_structured_output(_Statements)
	...
	def evaluate_faithfulness(
		...
		try:
			...
			stmt_obj = cast(_Statements, result)
		...
		if not has_context:
			...
			result = self.verify_no_context_chain.invoke({
				"question": question,
		...
		# ---------------------------------------------------------
			...
			result = self.verify_context_chain.invoke({
				"question": question,
				"context_str": context_str,
				"statements_str": statements_str,
			})

		verif_obj = cast(_Verification, result)
	...
	def _build_breakdown_chain(self):
		breakdown_prompt = """
Goal: Given a question and an answer, analyze the complexity of each sentence in the answer. Break down each sentence into one or more fully understandable statements. Ensure that no pronouns are used in any statement.
"""
	...
	def _build_classification_chain(self):
		classify_prompt = """Goal: Compare answer statements against ground truth statements using semantic meaning,
not exact wording.
...
"""
		...
		return prompt | self.client.with_structured_output(_Classification)
	...
	def evaluate_answer_correctness(
		...
		def _get_statements(text: str) -> list[str]:
			...
			result = self.breakdown_chain.invoke({
				"question": question,
				"clean": clean,
			})
			obj = cast(_Statements, result)
		...
		classif_obj = cast(_Classification, result)
		...
		return AnswerCorrectness(
			classifications=classif_obj.classifications,
			tp=tp,
			fp=fp,
			fn=fn,
			precision=precision,
	...
	def evaluate_results(self, results_df: pd.DataFrame, verbose: bool = False) -> pd.DataFrame:
		...
		for _, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
			...
			recall_scores.append(recall.recall)
			faithfulness_scores.append(faith.faithfulness)
			correctness_scores.append(corr.answer_correctness)
